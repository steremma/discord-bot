...
# Lol name malaka fortuna.
GUILD = 'group'
STATE_FILE = 'state.tsv.txt'

# Assuming Eulalia knows what he is talking about.
...
def check(tod):
    ...
    print("Start: ", start, "now", now, "end", end)
    print("Therefore", start <= now <= end)
    if start <= now <= end:
        return "On Window"
    if now < start:
        delta = start - now
